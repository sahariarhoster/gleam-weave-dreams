    ...
    <DialogContent className="max-w-2xl">
      ...
      <form onSubmit={(e) => { e.preventDefault(); mut.mutate(); }} className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="space-y-1.5"><Label>Min Delay (s)</Label>
            <Input type="number" min={0} value={form.min_delay_seconds} onChange={(e) => setForm({ ...form, min_delay_seconds: Number(e.target.value) })} />
          </div>
          <div className="space-y-1.5"><Label>Max Delay (s)</Label>
            <Input type="number" min={0} value={form.max_delay_seconds} onChange={(e) => setForm({ ...form, max_delay_seconds: Number(e.target.value) })} />
          </div>
          <div className="space-y-1.5"><Label>Daily Limit</Label>
            <Input type="number" min={1} value={form.daily_limit} onChange={(e) => setForm({ ...form, daily_limit: Number(e.target.value) })} />
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div className="space-y-1.5"><Label>Window Start (BD)</Label>
            <Input type="time" value={form.send_window_start} onChange={(e) => setForm({ ...form, send_window_start: e.target.value })} />
          </div>
          <div className="space-y-1.5"><Label>Window End (BD)</Label>
